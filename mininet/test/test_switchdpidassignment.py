...
class testSwitchDpidAssignment ( unittest.TestCase ):
    ...
    def testDefaultDpid ( self ):
        """Verify that default dpid is assigned using a valid provided
        canonical switchname if no dpid is not passed in switch creation."""
        mn = Mininet( Topo() )
        switch = mn.addSwitch( 's1' )
        self.assertEqual( switch.defaultDpid(), switch.dpid )

    def testActualDpidAssignment( self ):
        """Verify that Switch dpid is Actual dpid is assigned if dpid is passed
         in switch creation."""
        mn = Mininet( Topo() )
        switch = mn.addSwitch( 'A', dpid = '000000000000ABCD' )
        self.assertEqual( switch.dpid, '000000000000ABCD' )

    def testDefaultDpidAssigmentFailure( self ):
        """Verify that Default dpid assignment fails if the name of the switch
    ...
    def testDefaultDpidLen( self ):
        """Verify that Default dpid is lenght is 16 characters consisiting of
